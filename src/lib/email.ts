// Email utility for sending route links
// For now, we'll use a simple approach. You can integrate with Resend, SendGrid, or your preferred email service.
...
export async function sendEmail(options: EmailOptions): Promise<void> {
  ...
  //   from: 'South Bound <noreply@southbound.com>',
  //   to: options.to,
  ...
  // For development, you can use a service like Resend (free tier available)
  // npm install resend
  ...
  const resend = new Resend(process.env.RESEND_API_KEY);
  await resend.emails.send({
    from: process.env.EMAIL_FROM || 'South Bound <noreply@southbound.com>',
    to: options.to,
    subject: options.subject,
    html: options.html,
    text: options.text,
