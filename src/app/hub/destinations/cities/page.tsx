...
const guideCities = getAllCities();
      ...
      <div className="pt-4 border-t border-stone-200">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h2 className="text-xl font-bold text-stone-900 flex items-center gap-2">
              <BookOpen className="w-5 h-5 text-sb-orange-500" />
              Destination Guides
            </h2>
            <p className="text-stone-500 text-sm mt-1">
              Detailed guide content for the public destination pages — overview, food, activities, costs, visa, and more.
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {guideCities.map((city) => (
            <div
              key={city.slug}
              className="bg-white rounded-xl border border-stone-200 overflow-hidden hover:border-sb-orange-300 transition-colors group"
            >
              {/* Hero thumbnail */}
              <div className="relative h-28 bg-stone-100 overflow-hidden">
                <img
                  src={`${city.heroImage}&w=400&q=60`}
                  alt={city.name}
                  className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300"
                />
                <div className="absolute inset-0 bg-gradient-to-t from-stone-900/60 to-transparent" />
                <div className="absolute bottom-2 left-3 text-white">
                  <span className="text-lg mr-1">{city.flag}</span>
              ...
              {/* Info */}
              <div className="p-4">
                <p className="text-xs text-stone-500 mb-1">{city.country} · {city.region}</p>
                <p className="text-xs text-stone-600 line-clamp-2 leading-relaxed mb-3">
                  {city.tagline}
                </p>
                <div className="flex items-center gap-2 text-xs text-stone-500 mb-4">
                  <span>⭐ {city.quickStats.nomadRating}/10</span>
                  <span>·</span>
                  <span>{city.quickStats.monthlyBudget}</span>
                </div>
                <Link
                  href={`/hub/destinations/cities/guides/${city.slug}`}
                  className="flex items-center justify-center gap-2 w-full px-3 py-2 bg-sb-orange-500 hover:bg-sb-orange-600 text-white rounded-lg text-sm font-medium transition-colors"
                >
